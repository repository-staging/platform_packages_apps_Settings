...
public class UserAppsInstallSettings extends RadioButtonPickerFragment {
    ...
    private static String getCurrentInstallRestriction(
            UserRestrictionsFetcher userRestrictionsFetcher) {
        if (userRestrictionsFetcher.isSet(UserManager.DISALLOW_INSTALL_APPS)) {
            return INSTALL_DISABLED;
        }

        if (userRestrictionsFetcher.isSet(UserManager.DISALLOW_INSTALL_UNKNOWN_SOURCES)) {
    ...
    static String getDescription(Context context, UserRestrictionsFetcher userRestrictionsFetcher) {
        switch (getCurrentInstallRestriction(userRestrictionsFetcher)) {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mUserRestrictionsFetcher = UserRestrictionsFetcher.create(ctx, userId);
        if (mUserRestrictionsFetcher == null) {
    ...
    protected List<? extends CandidateInfo> getCandidates() {
        ...
        if (mUserRestrictionsFetcher.canSet(UserManager.DISALLOW_INSTALL_APPS)
                && mUserRestrictionsFetcher.canSet(UserManager.DISALLOW_INSTALL_UNKNOWN_SOURCES)) {
        ...
        if (mUserRestrictionsFetcher.canSet(UserManager.DISALLOW_INSTALL_APPS)) {
            candidates.add(new CandidateInfoExtra(
                    ctx.getString(R.string.user_app_install_enabled_first_party_sources),
    ...
    protected String getDefaultKey() {
        return getCurrentInstallRestriction(mUserRestrictionsFetcher);
    }
    ...
    protected boolean setDefaultKey(String key) {
        ...
        switch (key) {
            case INSTALL_ENABLED:
                mUserRestrictionsFetcher.set(UserManager.DISALLOW_INSTALL_APPS, false);
                mUserRestrictionsFetcher.set(UserManager.DISALLOW_INSTALL_UNKNOWN_SOURCES, false);
                return true;
            case INSTALL_FIRST_PARTY_ENABLED:
                mUserRestrictionsFetcher.set(UserManager.DISALLOW_INSTALL_APPS, false);
                mUserRestrictionsFetcher.set(UserManager.DISALLOW_INSTALL_UNKNOWN_SOURCES, true);
                return true;
            case INSTALL_DISABLED:
                mUserRestrictionsFetcher.set(UserManager.DISALLOW_INSTALL_APPS, true);
                mUserRestrictionsFetcher.set(UserManager.DISALLOW_INSTALL_UNKNOWN_SOURCES, true);
                return true;
            default:
                return false;
